...
import 'swiper/css';
import Api from '../utils/Api'; // Replace with the actual API import path
        ...
        const productData = async () => {
            try {
                ...
                const recentProducts = res.filter((product) => {
                    ...
                    return (
                        (currentTime > endTime || ['Sold', 'Withdrawn'].includes(product.status))
                    );
                });
                        ...
                        products.slice(0, 4).map(product => (
                            ...
                            <div key={product.id}>
                                <div className="bg-[#AD8B73] border-0 rounded-2xl relative overflow-hidden group cursor-pointer hover:scale-105 transition-all duration-300 h-full">
                                    <img src={Helper.BASE_URL + product.images[0] || img1} alt={product.name} />
                                    ...
                                    <div className="bg-[#212121] w-full px-5 py-3 text-white flex justify-between">
                                        ...
                                        <div className="flex flex-col">
                                            <div className="text-base">No. of Bids: {product.bids ? product.bids : '0'}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
